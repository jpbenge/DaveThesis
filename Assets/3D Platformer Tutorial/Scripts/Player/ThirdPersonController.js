...
private var activePlatform : GameObject;
private var platformYOffset : float = 0;
...
function UpdateSmoothedMovementDirection ()
{
	...
	if (grounded)
	{
		...
		{
			platformDif = (activePlatform.transform.position - lastActivePlatformPosition);
			if (platformDif.y > 0)
			{
				//platformDif = new Vector3(platformDif.x,platformDif.y*1.5f,platformDif.z);
			}
			lastActivePlatformPosition = activePlatform.transform.position
	;	}
		else
		{
			platformDif = Vector3.zero;
		}
...
function LateUpdate ()
{
	...
	{
		//Debug.Log("ADJUSTING");
		//transform.position = new Vector3(transform.position.x,activePlatform.transform.position.y+platformYOffset,transform.position.z);
	}
}
...
function OnControllerColliderHit (hit : ControllerColliderHit )
{
	...
	if (hit.gameObject.tag == "Moving Platform" && hit.moveDirection.y < -0.9 && hit.normal.y > 0.5)
	{
		activePlatform = hit.gameObject;
